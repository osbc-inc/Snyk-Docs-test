...
다음으로, 이전 모듈에서 생성한`initGlobalMiddlewares()` 함수를 수정하여 express에게 EJS와 같은 \_뷰 엔진\_을 사용하고 뷰 템플릿을 저장할 위치를 알려줍니다. 우리는 EJS 템플릿을 `./src/views`에 저장하고 이미지 및 CSS와 같은 일반적인 파일은 `./src/public`에 저장할 것입니다.
