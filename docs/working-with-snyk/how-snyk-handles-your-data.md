...
Snyk이 처리하는 데이터는 사용 중인 제품, Snyk과의 통합 방식 및 Snyk 배포에 따라 다양합니다. Snyk은 빠르게 변화함에 유의하십시오. 따라서, Snyk이 액세스하고 저장하는 데이터 유형은 새로운 기능 도입 또는 기존 기능에 대한 변경으로 변경될 수 있습니다.
...
{% hint style="info" %}
모든 데이터는 Snyk에서 SOC 2 표준에 따라 처리됩니다. 자세한 내용은 [Snyk 인증서](how-snyk-handles-your-data.md#snyk-1)를 참조하십시오.
{% endhint %}
...
Snyk은 데이터를 보호하는 것이 얼마나 중요한지 알고 있습니다. 따라서, Snyk은 각각의 Snyk 제품에 대해 설명된대로 정확한 분석을 제공하고 Snyk 서비스를 제공하기 위해 필요한 정보만 액세스하고 저장할 것입니다: Snyk Open Source, Snyk Code, Snyk Container, Snyk IaC.
