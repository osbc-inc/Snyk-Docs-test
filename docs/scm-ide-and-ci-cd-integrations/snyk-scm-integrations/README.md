...
Snyk를 귀하의 Git 저장소와 통합하여 귀하의 **Projects** 목록에 추가되는 모든 [Snyk 프로젝트](https://docs.snyk.io/introducing-snyk/introduction-to-snyk-projects)를 빠르고 쉽게 볼 수 있습니다.
...
Snyk 계정에 새로운 통합을 추가하려면 먼저 어느 수준 타입에서 통합을 설치할 것인지 결정해야 합니다. Snyk는 Snyk 계층의 다른 수준에서 다른 SCM 통합을 지원합니다:

- [그룹 수준](./#group-level-snyk-apprisk-integrations) - 그룹 수준 통합은 Snyk AppRisk Essentials 또는 Snyk AppRisk Pro만 지원합니다.
- [조직 수준](./#organization-level-snyk-integrations) - 조직 수준 통합은 다른 Snyk 제품을 지원합니다. Snyk는 조직 수준에서 Snyk AppRisk Essentials 또는 Snyk AppRisk Pro 통합을 지원하지 않습니다.
...
- **추가** 버튼을 클릭합니다.
- **Organizations** 필드에 `*` 기호를 사용하여 조직 세부 정보를 추가합니다. 예를 들어 `*snyk`를 사용하면 이름에 Snyk가 포함된 모든 SCM 조직이 통합됩니다.
...
Snyk는 GitHub, GitLab, Bitbucket, Azure Repos와 같은 다양한 소스 제어 관리 시스템과의 조직 수준에서 원활한 통합을 제공합니다. 이러한 통합을 통해 취약점을 자동으로 스캔하고 실행 가능한 통찰을 제공하여 귀하의 저장소의 보안을 강화할 수 있습니다. 이 수준에서 통합을 실행하여 조직 내의 모든 프로젝트에 대한 포괄적인 보호를 보장할 수 있습니다.
...
Snyk는 다음 SCMs와 통합하여 귀하의 코드에서의 문제와 취약점을 추적, 모니터링 및 수정할 수 있도록 지원합니다:
...
조직 전체에 Snyk를 원활하게 배포하려면 배포 단계, 구성 단계 및 각 단계의 원하는 결과로 구성된 Snyk의 제안된 배포 시간표를 제공합니다.
...
| --- | --- | --- |
| **일일/주간 테스트:**<br>개인 저장소에서 manifest 파일 읽기 | <code>repo (모두)</code> | ≥ <code>read</code> |
| **수동 고치기 PR:**<br>감시 중인 저장소에 고치기 PR 생성 | <code>repo (모두)</code> | |
| **자동 고치기 및 업그레이드 PR:**<br>감시 중인 저장소에 고치기 또는 업그레이드 PR 생성 | <code>repo (모두)</code> | ≥ <code>write</code> |
| **풀 리퀘스트 대한 Snyk 테스트:**<br>새 PR이 만들어지거나 기존 PR이 업데이트될 때 PR 상태 확인 전달 | <code>repo (모두)</code> | ≥ <code>write</code> |
| **풀 리퀘스트에 대한 Snyk 테스트 초기 구성:**<br>수입된 저장소에 SCM 웹훅 추가 | <code>admin:repo_hooks (read &#x26; write)</code> | <code>admin</code> |
| **새 프로젝트를 Snyk에 가져오기:**<br>GitHub org에 있는 모든 사용 가능한 저장소 목록을 <strong>프로젝트 추가</strong> 화면에 제시 | <code>admin:read:org</code><br><code>repo (모두)</code> | |
...
PAT의 `Administration: Read-only` 권한은 Snyk가 새 프로젝트를 가져오기 위해 필요한 사용자의 액세스 가능한 GitHub 조직을 식별하고 나열하는 데 중요합니다.

Snyk는 병합이 발생할 것을 [GitHub Enterprise](github-enterprise.md)에 알리기 위해 PR을 사용합니다. 이를 위해 콘텐츠가 분기로 푸시되어야 하며 이렇게 되면 `content: write` 범위가 필요합니다. 그런 다음 따로 호출을 통해 고칠 PR을 만들고 이는 `pull request: write` 범위가 필요합니다. 그런 후 GitHub Enterprise에게 PR을 만들도록 지시하여 변경된 브랜치를 기본 브랜치로 병합합니다.
...
| --- | --- | --- |
| **일일/주간 테스트:**<br>개인 저장소에서 manifest 파일 읽고 private repos에서 파일 수정 | **저장소:** `읽기` | ≥ `쓰기` |
| **수동 고치기 PR (사용자 트리거):**<br>저장소에 고치기 PR 생성 | **저장소**: `읽기, 쓰기`<br>**풀 리퀘스트**: `읽기, 쓰기` | |
| **자동 고치기 및 업그레이드 PR:**<br>저장소에서 고치기/업그레이드 PR 생성 | **저장소**: `읽기, 쓰기`<br>**풀 리퀘스트**: `읽기, 쓰기` | ≥ `쓰기` |
| **풀 리퀘스트에 대한 Snyk 테스트:**<br>새 PR이 만들어지거나 PR이 업데이트될 때 PR 상태 확인 전달 | **저장소**: `읽기, 쓰기`<br>**풀 리퀘스트**: `읽기, 쓰기` | ≥ `쓰기` |
| **풀 리퀘스트에 대한 Snyk 테스트 (초기 설정):**<br>수입된 저장소에 SCM 웹훅 추가 | **웹훅**: `읽기, 쓰기` | `관리자` |
| **Snyk에 새 프로젝트 가져오기:**<br>Bitbucket 인스턴스에 있는 모든 사용 가능한 저장소 목록을 <strong>프로젝트 추가</strong> 화면에 제시 | **계정**: `읽기`<br>**워크스페이스 멤버십**: `읽기`<br>**프로젝트**: `읽기` | |
