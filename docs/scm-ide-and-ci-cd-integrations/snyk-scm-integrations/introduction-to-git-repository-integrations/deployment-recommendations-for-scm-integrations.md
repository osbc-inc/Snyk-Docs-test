...
- 최신 버전이 이미 프로젝트에서 발견되지 않은 취약점을 포함하는 경우, Snyk는 업그레이드를 권장하지 않습니다.
- Snyk는 21일 미만의 버전을 업그레이드하는 것을 권장하지 않습니다. 이는 기능 버그를 도입하고 나중에 게시가 취소되는 버전이거나, 권한이 이상한 의도로 전환되는 계정에서 출시된 버전을 피하기 위함입니다.

### **지원되는 언어 및 저장소**

Snyk은 GitHub, GitHub Enterprise Server 및 BitBucket Cloud를 통해 npm, Yarn 및 Maven-Central 프로젝트에 대한 자동 업그레이드 PR을 지원합니다. Snyk 브로커와 함께 사용이 가능합니다. 브로커와 함께 사용하려면 관리자에게 먼저 v4.55.0 이상으로 업그레이드하라고 요청하세요.

### **특정 프로젝트에 대한 자동 의존성 업그레이드 PR 활성화**

프로젝트 수준에서 PR 설정을 설정하고 조직 수준에서 구성된 PR 설정을 재정의하려면:

1. 자동 업그레이드 PR을 활성화하려는 조직을 열고 **프로젝트** 탭으로 이동합니다.
2. 해당 프로젝트를 선택하고 확장하고, 관련 대상을 선택하고, 톱니바퀴 아이콘을 사용하여 **설정**을 엽니다.
3. 설정 영역에서 왼쪽 패널 메뉴에서 해당 특정 프로젝트에 대한 고유 설정을 적용하려면 **통합 설정**을 클릭합니다.
4. 설정이 로드되면 **자동 의존성 업그레이드 풀 리퀘스트**로 스크롤하고 **비활성화**를 클릭합니다.
5. 나타나는 옵션에서:
   1. Snyk는 레포지토리 당 최대 열 개의 PR을 만듭니다.\
      이 번호를 더 제한하려면 드롭다운 목록에서 PR의 최대 개수를 선택합니다.\
      자세한 내용은 [자동 PR을 사용하여 종속성 업그레이드](../../../scan-with-snyk/pull-requests/snyk-pull-or-merge-requests/upgrade-dependencies-with-automatic-prs-upgrade-prs/)를 참조하세요.
   2. **무시할 종속성** 필드에 자동 기능으로 처리되어서는 _안 되는_ 종속성의 정확한 이름을 입력합니다.\
      이 필드는 소문자만 허용합니다.
   3. **의존성 업그레이드 설정**을 클릭한 후, 각각의 시간에 Snyk가 이 프로젝트를 스캔할 때마다, Snyk는 결과에 따라 자동으로 업그레이드 PR을 제출합니다.\
      기존의 Snyk 업그레이드 PR이나 기존의 수정 PR에 대해 더 새로운 버전이 출시된 경우, Snyk가 새 PR을 생성하기 전에 기존 PR을 닫거나 병합해야 합니다.