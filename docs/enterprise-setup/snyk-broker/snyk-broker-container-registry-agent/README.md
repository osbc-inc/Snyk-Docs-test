...
컨테이너 레지스트리 에이전트를 사용하면 Snyk가 호스팅하는 개인 컨테이너 레지스트리를 통합할 수 있으며 이를 통해 해당 레지스트리 내의 컨테이너 이미지를 보다 안전하게 보호할 수 있습니다. 개인 컨테이너 레지스트리와의 통합을 통해 다음을 수행할 수 있습니다:

- 액세스 토큰과 같은 중요한 데이터를 개인 네트워크 내부에 보관하여 해당 정보를 Snyk와 공유하지 않습니다.
- 네트워크에 대한 Snyk 액세스를 제한하고, Snyk가 수행할 수 있는 작업을 제한하기 위해 Snyk에 대한 제어된 액세스를 제공합니다.
...
<figure><img src="../../../.gitbook/assets/mceclip0-8-.png" alt="Snyk 브로커 컨테이너 레지스트리 에이전트의 고수준 아키텍처"></figcaption></figure>

## 지원되는 컨테이너 레지스트리들

Snyk 브로커 컨테이너 레지스트리 에이전트를 사용하여 Snyk를 다음 오픈 소스 컨테이너 레지스트리와 통합할 수 있습니다:
...
- 컨테이너 레지스트리 에이전트 머신 시스템 요구 사항 (MAX_ACTIVE_OPERATIONS=1 지정):
  - CPU: 1 vCPU
...
Docker 허브에서 컨테이너 레지스트리 에이전트 이미지를 다운로드할 수 있습니다. [전제 조건](./#prerequisites-for-container-registry-agent)에서 제공된 링크를 사용하십시오.

컨테이너 레지스트리 에이전트를 구성하려면 다음 환경 변수가 필요합니다:

- `SNYK_PORT` - 컨테이너 레지스트리 에이전트가 연결을 수락하는 로컬 포트 (기본값: 17500).
- `SNY## 시스템 점검(systemcheck) 사용하기

Broker Client의 `/systemcheck` 엔드포인트를 사용하여 Broker Client, 컨테이너 레지스트리 에이전트 및 컨테이너 레지스트리 간의 연결을 확인할 수 있습니다.

엔드포인트를 사용하려면 다음 환경 변수를 Broker Client에 제공하십시오:\
`BROKER_CLIENT_VALIDATION_URL=<agent-url>/systemcheck`

Broker Client의 `/systemcheck` 엔드포인트를 호출하면, `BROKER_CLIENT_VALIDATION_URL`을 사용하여 컨테이너 레지스트리 에이전트의 `/systemcheck` 엔드포인트에 요청을 보내며, 이때 토큰도 함께 제공됩니다. 컨테이너 레지스트리 에이전트는 컨테이너 레지스트리로의 연결 유효성을 검증하기 위해 요청을 보냅니다.

{% hint style="info" %}
`/systemcheck` 엔드포인트는 브로커 통합(Integration) 기능에 필수적인 것은 아닙니다. 자세한 정보는 [시스템 점검 문서](../troubleshooting-broker.md#monitoring-systemcheck)를 참조하십시오.
{% endhint %}

## 컨테이너 레지스트리 에이전트의 디버깅 방법

`LOG_LEVEL` 환경 변수를 설정하여 컨테이너 레지스트리 에이전트 및 Broker Client의 원하는 로그 레벨을 제어할 수 있습니다. 기본값은 `info`이며, 허용된 값은 `debug`, `info`, `warn`, `error`입니다.

{% hint style="warning" %}
제 3자 라이브러리에 대한 디버깅을 활성화하는 것은 프로덕션 환경에서 권장되지 않습니다. 왜냐하면 이로 인해 로그에 미유지된 민감한 정보(예: HTTP 요청의 헤더)가 노출될 수 있습니다.
{% endhint %}

제 3자 HTTP 요청 라이브러리 [Needle](https://www.npmjs.com/package/needle)로부터 디버그 로그를 활성화하려면 `NODE_DEBUG=needle` 환경 변수를 설정하십시오.

모든 제 3자 라이브러리로부터 디버그 로그를 활성화하려면 환경 변수 `DEBUG=*` 및 `NODE_DEBUG=*`를 설정하십시오.

`DEBUG` 환경 변수는 [Debug](https://www.npmjs.com/package/debug) 패키지 출력을 제어하며, `NODE_DEBUG` 환경 변수는 Node.js [util.debuglog](https://nodejs.org/api/util.html#utildebuglogsection-callback) 출력을 제어합니다.