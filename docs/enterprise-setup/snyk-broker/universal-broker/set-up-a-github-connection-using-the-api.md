...
이렇게 DEPLOYMENT\_ID (data.id)를 반환합니다. 예를 들면:
...
이 시점에서 Broker 클라이언트 실행을 시작할 수 있습니다.

## Broker 배포 실행 <a href="#run-your-broker-deployment_1" id="run-your-broker-deployment_1"></a>

필요에 따라 일반적인 `-e BROKER_SERVER_URL=https://broker.REGION.snyk.io \`로 원하는 환경을 타깃팅합니다.
...
## 자격 증명 참조 또는 참조 생성 <a href="#id-3-create-your-credentials-references" id="id-3-create-your-credentials-references"></a>
...
자격 증명 참조 ID (data. id)가 반환됩니다. 예를 들면:
...
여러 개의 객체를 속성에 더하여 한 번에 최대 열 개의 자격 증명을 생성할 수 있습니다. 이러한 객체는 다른 유형일 수 있습니다.
...
연결 ID (data.id)가 반환됩니다. 예시에서 보시듯이, 이 호출은 cred 레퍼런스 UUID 대신 사용할 수 있도록 자격 증명 참조를 직접 반환합니다.
...
자격 증명 참조가 빠져 있는 경우 다음과 같은 메시지를 받게 됩니다:
