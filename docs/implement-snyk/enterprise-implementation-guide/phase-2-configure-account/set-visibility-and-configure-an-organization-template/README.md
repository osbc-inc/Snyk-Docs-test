...
* 제품 설정, 예를 들어 {{Snyk Code}}를 활성화하는 것.
  * 새 조직에는 기본적으로 {{Snyk Code}}가 비활성화되어 있습니다.
  * Git 리포지토리 통합을 통해 가져온 저장소에서 SAST 스캔을 활성화하려면, 프로젝트를 가져오기 전에 {{Snyk Code}}를 활성화하도록 토글을 사용하십시오.
  * 만약 {{Snyk Code}}를 사용하고 **프로젝트를 가져오기 전에 활성화를 잊는다면, {{Snyk Code}}를 활성화하고 Git에서 코드를 다시 가져와야 합니다**. [{Snyk Code 활성화](enable-snyk-code.md)를 참조하십시오.
* 알림 설정 (이메일 알림)
