...
`snyk-iac-rules` - 를 위한 사용자 지정 규칙을 작성, 디버그, 테스트 및 번들로 제작하는 SDK
...
이 SDK는 Rego로 작성된 사용자 지정 규칙을 작성, 디버그, 테스트, 번들 제작 및 배포하는 데 도움이 되며, 이러한 규칙은  CLI에서 IaC 구성 파일의 취약점을 찾을 수 있도록 사용할 수 있습니다.
...
번들에 사용할 형식입니다. Snyk는 현재  CLI에서 Rego 번들을 지원하지 않습니다.
