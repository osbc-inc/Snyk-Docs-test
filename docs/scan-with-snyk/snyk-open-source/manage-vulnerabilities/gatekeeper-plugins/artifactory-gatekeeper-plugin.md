...
Artifactory **원격 저장소**에서 패키지 관리자 또는 URL에서 다운로드 요청이 발생할 때마다, Snyk은 아티팩트를 자동으로 취약점과 라이선스 문제에 대해 스캔합니다. Snyk이 발견한 문제는 아티팩트 속성으로 유지됩니다. 이러한 속성을 기반으로 나중에 패키지에 대한 액세스가 제어되며, 플러그인 구성에서 설정된 심각성 임계값에 따라 조정됩니다.
