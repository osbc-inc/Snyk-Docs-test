...
* **SCM 통합**: lockfile이 없는 경우, Snyk은 빌드 시점에서 트리가 어떻게 보일지 대략적으로 계산합니다. 이는 lockfile이 없을 때 개발 중인 프로젝트나 다음 빌드가 어떻게 보일지에 대한 통찰을 제공하는 데 매우 유용합니다

npm 사용자로서, 종속성을 처리할 때 언제든지 npm-audit이 사용 가능한 상황에서 '**Snyk** 왜?'할 수 있습니다. 다음 능력을 제공받습니다:

* Snyk은 오픈 소스 뿐만 아니라 당신의 제1 자 코드를 안전하게 보호합니다. 만약 인프라스트럭처 as 코드 및/또는 컨테이너를 사용 중이라면 Snyk은 가시성과 해결책을 제공할 수 있습니다.
...
자세한 내용은 [npm용 Snyk](./#npm)을을 참조하세요.
